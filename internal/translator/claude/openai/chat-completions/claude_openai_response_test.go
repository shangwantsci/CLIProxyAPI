...
func TestConvertClaudeResponseToOpenAI_StreamRefusalMapsContentFilter(t *testing.T) {
	ctx := context.Background()
	var param any
	...
	if got := gjson.GetBytes(out[0], "choices.0.finish_reason").String(); got != "content_filter" {
		t.Fatalf("finish_reason = %q, want content_filter; chunk=%s", got, string(out[0]))
	}
}

func TestConvertClaudeResponseToOpenAINonStream_IgnoresFallbackContentBlock(t *testing.T) {
	ctx := context.Background()
	...
	out := ConvertClaudeResponseToOpenAINonStream(ctx, "claude-fable-5", nil, nil, raw, nil)
	if got := gjson.GetBytes(out, "choices.0.message.content").String(); got != "ok" {
		t.Fatalf("content = %q, want ok; body=%s", got, string(out))
	}
	if gjson.GetBytes(out, "choices.0.message.tool_calls").Exists() {
		t.Fatalf("fallback block should not create tool_calls; body=%s", string(out))
	}
}
...
func TestConvertClaudeResponseToOpenAINonStream_PreservesClaudeCacheBreakdownWhenBillableInputEnabled(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "usage.prompt_tokens").Int(); got != 13 {
		t.Fatalf("prompt_tokens = %d, want upstream uncached input 13; out=%s", got, string(out))
	}
	...
	if got := gjson.GetBytes(out, "usage.total_tokens").Int(); got != 17 {
		t.Fatalf("total_tokens = %d, want 17; out=%s", got, string(out))
	}
}
