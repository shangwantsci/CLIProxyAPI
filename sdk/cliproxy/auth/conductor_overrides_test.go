...
func TestManager_DisablesClaudeWhenOAuthNotAllowedForOrganization(t *testing.T) {
	m := NewManager(nil, nil, nil)

	executor := &authFallbackExecutor{
		id: "claude",
		executeErrors: map[string]error{
			"claude-oauth-not-allowed": &Error{
				HTTPStatus: http.StatusForbidden,
				Message:    `{"type":"error","error":{"type":"permission_error","message":"OAuth authentication is currently not allowed for this organization."},"request_id":"req_123"}`,
			},
		},
	}
	m.RegisterExecutor(executor)
	...
	model := "claude-sonnet-4-5"
	auth := &Auth{ID: "claude-oauth-not-allowed", Provider: "claude"}
	reg := registry.GetGlobalRegistry()
	reg.RegisterClient(auth.ID, "claude", []*registry.ModelInfo{{ID: model}})
	t.Cleanup(func() {
		reg.UnregisterClient(auth.ID)
	...
	if updated.LastError.Code != "organization_disabled" || updated.LastError.HTTPStatus != http.StatusForbidden {
...
func TestManager_MarkResult_ClientRequestBadRequestDoesNotPolluteAuthHealth(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "unsupported effort level",
			message: "400 / level *xhigh* not supported, valid levels: low, medium, high, max",
		},
		{
			name:    "budget out of range",
			message: "budget 200000 out of range [1024,128000]",
		},
		{
			name:    "thinking unsupported",
			message: "thinking not supported for this model",
		},
		{
			name:    "unknown thinking level",
			message: "unknown level: ultra",
		},
	}
