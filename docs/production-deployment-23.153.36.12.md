...
- 后端提交：`012545f6`（session_key 兜底重认证 P2 + 认证失效清残留 quota P3 后端；只更新后端）
- 前端提交：`14204dc`（本次未改前端，保留服务器现有 management.html）
- 最近一次按本文档部署时间：`2026-06-01T12:20:08+00:00`
- 本次部署后端二进制 sha256：`cd70e17b3ca8eb3fddef19ac10162678a7ad19611e95d5ff258faf3cf8eb043e`
- 部署前运行版本 backend=`27f61fa7`，binary sha256 `7c782256…aa8e`
- 部署前后端二进制备份：`/opt/cpa-claude-proxy-backups/CLIProxyAPI-before-deploy-20260601-121405.bak`（可回滚到 `27f61fa7`）

> ⚠️ **账号数据告警(2026-06-01)**：本次部署时发现账号池已从 5-31 的 92 个骤降到 **5 个 claude 账号**，且降幅发生在本次部署**之前**(剩余文件 mtime 10:54–11:02、auths 目录 mtime 10:38，均早于本次部署的 12:13)。**原因未查清**，非本次部署造成。
> - 本次部署当天的备份 `auths-20260601-121405.tgz` **只含 5 个账号，已失效**，不可用于恢复。
> - **目前最完整的账号恢复点是上一轮备份 `/opt/cpa-claude-proxy-backups/auths-20260531-061110.tgz`(92 账号)**；但其中大部分是被上游封控(P1)的 team org 账号。
> - 账号大规模丢失作为独立问题待排查(查更早日志 / auths 目录变更史 / 是否有删除操作或清理逻辑触发)。恢复账号前应先定位丢失根因，避免恢复后再次被同样原因清除。
